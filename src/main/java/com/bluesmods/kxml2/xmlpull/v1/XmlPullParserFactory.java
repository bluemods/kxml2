...
package com.bluesmods.kxml2.xmlpull.v1;
...
public class XmlPullParserFactory {
    ...
    protected ArrayList<Class<?>> parserClasses;
    protected ArrayList<Class<?>> serializerClasses;
    ...
    protected XmlPullParserFactory() {
        parserClasses = new ArrayList<>();
        serializerClasses = new ArrayList<>();

//        BLUE-CHANGED: do not use Android's built-in classes, ever. Use our own
//        try {
...
//        }
    ...
    private XmlPullParser getParserInstance() throws XmlPullParserException {
        ...
        if (parserClasses != null && !parserClasses.isEmpty()) {
            ...
            for (Class<?> o : parserClasses) {
                try {
                    if (o != null) {
                        return (XmlPullParser) o.newInstance();
                    }
    ...
    private XmlSerializer getSerializerInstance() throws XmlPullParserException {
        ...
        if (serializerClasses != null && !serializerClasses.isEmpty()) {
            exceptions = new ArrayList<>();
            for (Class<?> o : serializerClasses) {
                try {
                    if (o != null) {
                        return (XmlSerializer) o.newInstance();
                    }
                } catch (InstantiationException | IllegalAccessException | ClassCastException e) {
                    exceptions.add(e);
                }
